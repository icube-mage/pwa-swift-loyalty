...
const ForgotPassword = (props) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object().shape({
            ...
            phoneNumberEmail:
                useForgotWithPhone
                && !data.otpConfigLoyalty.otp_enable[0].enable_otp_forgot_password
                && Yup.string()
        ...
        onSubmit: (values) => {
            ...
            getToken({
                ...
                .then((res) => {
                    setLoad(false);
                    const { token, message } = res.data.requestLinkForgotPasswordLoyalty;
                    if (token) {
                        setToast({
                            open: true,
                            variant: 'success',
    ...
    const handleSwitch = () => {
        ...
        if (data && data.otpConfigLoyalty.otp_enable[0].enable_otp_forgot_password) {
            setDisabled(!disabled);
        }
    };

    React.useEffect(() => {
        if (data && !data.otpConfigLoyalty.otp_enable[0].enable_otp_forgot_password && !useForgotWithPhone) {
            setUseEmail(true);
        } else if (data && data.otpConfigLoyalty.otp_enable[0].enable_otp_forgot_password) {
